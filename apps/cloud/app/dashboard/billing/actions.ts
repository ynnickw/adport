...
import { getOrganizationEntitlement, type PlanId } from '@/lib/cloud/plans';
...
export async function startSubscription(plan: PlanId): Promise<void> {
  ...
  const session = await stripeClient().checkout.sessions.create({
    ...
    metadata: { organizationId: principal.organizationId, plan },
    subscription_data: { metadata: { organizationId: principal.organizationId, plan } },
