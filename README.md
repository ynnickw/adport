...
The cloud application in `apps/cloud` is a separate flow. Google, Meta, TikTok, Microsoft, and Reddit connect through a hosted browser-based OAuth broker backed by Adport-owned, reviewed applications; tenants never paste application secrets, and each provider card activates once its application is approved and configured. Apple Ads uses OAuth 2.0 client credentials instead: each tenant supplies its Apple Ads API-user identifiers and ES256 private key, which Adport encrypts and uses to mint short-lived access tokens. All six providers share an encrypted tenant vault and the remote REST/MCP runtime.
